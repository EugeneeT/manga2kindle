...
const createImageSettingsController = (imageSettingsService) => {
  ...
  router.delete("/presets/:name", authMiddleware, async (req, res) => {
    try {
  ...
  router.post("/active-preset", authMiddleware, async (req, res) => {
    try {
      ...
      console.error("Error in POST /api/image-settings/active-preset:", error);
      res.status(500).json({
        success: false,
        error: error.message,
